	...
	<div class="relative flex h-full w-full">
		<div class="scrollbar-primary flex h-full w-full flex-col overflow-y-auto">
			<div class="relative h-full w-full lg:flex">
				...
				{/if}
				<div
					class="block h-full w-full overflow-y-auto border-r border-border px-5 pb-30 lg:max-h-full lg:bg-background lg:px-8 lg:py-8 lg:pt-10"
				>
					<div class="flex w-full flex-col gap-8">
						<div class="rounded-3xl bg-muted p-0 lg:p-7">
							{#if selectedMenu?.child && selectedMenu.child?.length > 0}
								<div class="relative w-full max-w-114.5">
									<UnderlineTabs.Root value={selectedTab}>
										<UnderlineTabs.List>
											{#each selectedMenu.child as item, index (index)}
												<UnderlineTabs.Trigger value={item.title}>
													{item.title}
												</UnderlineTabs.Trigger>
											{/each}
										</UnderlineTabs.List>
										<UnderlineTabs.Content value={selectedTab}></UnderlineTabs.Content>
									</UnderlineTabs.Root>
								</div>
							{/if}
						</div>
